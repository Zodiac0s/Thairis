// Esperar a que la pantalla cargue completamente antes de ejecutar eventos
document.addEventListener('DOMContentLoaded', () => {
    ...
    // EVENTO DEL BOTÓN CONTINUAR (MÓDULO 1 -> MÓDULO 2)
    if (btnContinuar) {
        const pasarAModulo2 = (e) => {
        ...
        btnContinuar.addEventListener('click', pasarAModulo2);
        btnContinuar.addEventListener('touchend', pasarAModulo2);
    }

    // EVENTO DEL BOTÓN IR A LA CARTA (MÓDULO 2 -> MÓDULO 3)
    if (btnIrCarta) {
        const pasarAModulo3 = (e) => {
            if (e) e.preventDefault();
            if (modulo2 && modulo3) {
        ...
        btnIrCarta.addEventListener('click', pasarAModulo3);
        btnIrCarta.addEventListener('touchend', pasarAModulo3);
    }

    // 3. SOBRE Y CARTA (MÓDULO 3)
    ...
    if (envoltorioSobre) {
        envoltorioSobre.addEventListener('click', () => {
            if (!envoltorioSobre.classList.contains('abierto')) {
                ...
                if (instruccionSobre) instruccionSobre.style.opacity = '0';

                setTimeout(() => {
                    envoltorioSobre.classList.add('ocultar-sobre');
                    if (hojaCarta) hojaCarta.classList.add('desplegada');
                    if (btnReiniciar) {
    ...
    if (btnCerrarCarta) {
        btnCerrarCarta.addEventListener('click', (e) => {
            e.stopPropagation();
            if (hojaCarta) hojaCarta.classList.remove('desplegada');
            
            setTimeout(() => {
                if (envoltorioSobre) envoltorioSobre.classList.remove('ocultar-sobre');
            }, 300);
        });
    }
    ...
    if (btnReiniciar) {
        btnReiniciar.addEventListener('click', (e) => {
            e.preventDefault();
            if (hojaCarta) hojaCarta.classList.remove('desplegada');
            if (envoltorioSobre) envoltorioSobre.classList.remove('abierto', 'ocultar-sobre');
            if (instruccionSobre) instruccionSobre.style.opacity = '1';
    ...
    // 4. CÁLCULO DEL UNIVERSO 3D (78 MENSAJES)
